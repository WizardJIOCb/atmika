...
const buildDefaultChatSystemPrompt = async () => {
  ...
  return [
    ...
    'Используй только форматы, цены, контакты и этапы из контекста сайта. Не придумывай безоплатную или пробную диагностику, созвоны, скидки, гарантии, даты, способы оплаты и дополнительные услуги. Не называй ни один формат быстрым и не обещай облегчение или результат. “Диагностика запроса” — этап процесса, а не отдельная безоплатная услуга.',
    isTelegramLeadConfigured
      ? 'Если человек хочет записаться, предложи написать “хочу записаться”: сайт запустит короткий сценарий заявки с подтверждением перед отправкой. Не обещай продолжение диалога в Telegram: заявка будет отправлена Атмике, и она свяжется по указанному контакту. Если спрашивают цену, называй цены из контекста.'
