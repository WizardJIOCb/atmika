(() => {
  ...
  const renderOverview = () => `
    ...
    ${categories().length ? `<div class="academy-category-admin-grid">${categories().map((entry) => {
      ...
      return `<article class="academy-admin-card"><button type="button" data-open-entity="category:${entry.id}"><span>${entry.published ? 'Опубликована' : 'Черновик'}</span><strong>${escapeHtml(entry.title)}</strong><small>${categoryCourses.length} формат(ов)</small></button><div>${categoryCourses.map((item) => `<button type="button" data-open-entity="course:${item.id}">${escapeHtml(item.title)} <em>${item.accessType === 'paid' ? money(item.priceKopecks) : '<s>Бесплатно</s> Безоплатно'}</em></button>`).join('') || '<small>Форматов пока нет</small>'}</div></article>`;
