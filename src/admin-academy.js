(() => {
  ...
  const renderCourseEditor = (item) => `
    ...
    <section class="academy-form-section"><div class="academy-field-grid">
      ...
      <label class="academy-field"><span>Доступ</span><select data-entity-field="accessType"><option value="free" ${item.accessType === 'free' ? 'selected' : ''}>Безоплатный</option><option value="paid" ${item.accessType === 'paid' ? 'selected' : ''}>Платный</option></select></label>
  ...
  const renderMaterialEditor = (item) => {
    ...
    return `
      ...
      <section class="academy-form-section"><div class="academy-field-grid">
        ...
        <label class="academy-field"><span>Доступ</span><select data-entity-field="accessType"><option value="free" ${item.accessType === 'free' ? 'selected' : ''}>Безоплатный материал</option><option value="paid" ${item.accessType === 'paid' ? 'selected' : ''}>Платный материал</option></select></label>
  ...
  const renderOverview = () => `
    ...
    ${categories().length ? `<div class="academy-category-admin-grid">${categories().map((entry) => {
      ...
      return `<article class="academy-admin-card"><button type="button" data-open-entity="category:${entry.id}"><span>${entry.published ? 'Опубликована' : 'Черновик'}</span><strong>${escapeHtml(entry.title)}</strong><small>${categoryCourses.length} формат(ов)</small></button><div>${categoryCourses.map((item) => `<button type="button" data-open-entity="course:${item.id}">${escapeHtml(item.title)} <em>${item.accessType === 'paid' ? money(item.priceKopecks) : 'Безоплатно'}</em></button>`).join('') || '<small>Форматов пока нет</small>'}</div></article>`;
