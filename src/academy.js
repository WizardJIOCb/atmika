(() => {
  ...
  const generatedPrivacyPolicy = (settings) => {
    ...
    const version = escapeHtml(settings.legalDocumentVersion || '18.07.2026-r2');
    return `<h1>Политика оператора в отношении обработки персональных данных</h1>
      <p><strong>Редакция документа: ${version}</strong></p>
      ...
      <p>При посещении сайта автоматически подключается Яндекс Метрика и обрабатываются метрические и технические данные о посещениях, включая источник перехода, просмотренные страницы, действия на сайте, тип устройства и браузера, а также идентификаторы аналитических cookies. Параметры запросов из адреса страницы в Метрику не передаются, запись сеанса Вебвизором отключена. Цель — анализ посещаемости и совершенствование работы сайта. Основание — законный интерес Оператора при условии соблюдения прав и свобод посетителей.</p>
      ...
      <p>3.2. Оператор не распространяет персональные данные неопределённому кругу лиц. В необходимом объёме данные могут предоставляться хостинг- и техническим подрядчикам, платёжному сервису ЮKassa, оператору фискальных данных, сервису Яндекс Метрика, а также используемым сервисам мессенджера и генеративного ИИ при отправке заявки или сообщения. Такие лица обрабатывают данные для работы соответствующей функции или на основании собственных правил.</p>
      ...
      <p>4.1. Необходимые технологии используются для авторизации, безопасности, работы личного кабинета и оплаты, сохранения идентификатора чата и выбранного облика помощника. Сессионный cookie личного кабинета может храниться до 30 дней. Отключение необходимых технологий может нарушить работу этих функций.</p>
      <p>4.2. Яндекс Метрика подключается автоматически и использует cookies и localStorage для обезличенной статистики посещаемости. Пользователь может ограничить или удалить такие идентификаторы средствами браузера, блокировщиком содержимого либо настройками самого сервиса Яндекс Метрика. Ограничение аналитических cookies не препятствует просмотру основных страниц сайта.</p>
      <h2>5. Сроки хранения и прекращение обработки</h2>
      <ul><li>данные аккаунта и доступов — пока действует аккаунт и сохраняются договорные обязательства;</li><li>сведения о заказах, платежах и юридически значимых действиях — в течение сроков, установленных договорным, бухгалтерским, налоговым и иным применимым законодательством, как правило не менее 5 лет;</li><li>обращения, заявки и переписка — до завершения работы с обращением либо отзыва согласия, если более длительное хранение не требуется по закону или договору;</li><li>технические журналы — не дольше периода, необходимого для безопасности и расследования сбоев;</li><li>cookies и localStorage — до истечения срока соответствующего идентификатора либо его удаления пользователем.</li></ul>
  ...
  const generatedPersonalDataConsent = (settings) => {
    ...
    return `<h1>Согласие на обработку персональных данных</h1>
      ...
      <p>Для указанных целей данные в необходимом объёме могут быть предоставлены хостинг- и техническим подрядчикам, ЮKassa и оператору фискальных данных, а при использовании соответствующих функций — мессенджеру Telegram и поставщику сервиса генеративного ИИ. Я понимаю, что мессенджер и сервис ИИ могут использовать инфраструктуру за пределами Российской Федерации. Использование Яндекс Метрики и аналитических cookies отдельно описано в Политике Оператора и не зависит от отметки под настоящей формой.</p>
